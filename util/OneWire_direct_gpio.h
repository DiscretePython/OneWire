...
// resolve any problems related to ESP chips.  Please do not contact me and
// please DO NOT CREATE GITHUB ISSUES for ESP support.  All ESP questions must
// be asked on ESP community forums.
...
directModeInput(IO_REG_TYPE mask) {
  if (mask > 0x8000) {
...
static inline __attribute__((always_inline)) void
directModeOutput(IO_REG_TYPE mask) {
  if (mask > 0x8000) {
    GP16FFS(GPFFS_GPIO(16));
    GPC16 = 0;
    GP16E |= 1;
...
#define DIRECT_WRITE_LOW(base, mask)                                           \
  (mask > 0x8000) ? GP16O &= ~1 : (GPOC = (mask))
#define DIRECT_WRITE_HIGH(base, mask)                                          \
  (mask > 0x8000) ? GP16O |= 1 : (GPOS = (mask))
...
static inline __attribute__((always_inline)) void
directWriteLow(IO_REG_TYPE pin) {
#if CONFIG_IDF_TARGET_ESP32C3 || CONFIG_IDF_TARGET_ESP32C6
...
directModeInput(IO_REG_TYPE pin) {
  ...
  if (digitalPinIsValid(pin)) {
    ...
    if (rtc_reg) // RTC pins PULL settings
    {
      ...
      ESP_REG(rtc_reg) = ESP_REG(rtc_reg) & ~(rtc_gpio_desc[pin].pullup |
...
#define IO_REG_BASE_ATTR
#define IO_REG_MASK_ATTR
#define DIRECT_READ(base, pin)                                                 \
  CY_SYS_PINS_READ_PIN(PORT_ADDRESS(pin) + 4, PIN_IN_PORT(pin))
#define DIRECT_WRITE_LOW(base, pin)                                            \
  CY_SYS_PINS_CLEAR_PIN(PORT_ADDRESS(pin), PIN_IN_PORT(pin))
#define DIRECT_WRITE_HIGH(base, pin)                                           \
  CY_SYS_PINS_SET_PIN(PORT_ADDRESS(pin), PIN_IN_PORT(pin))
#define DIRECT_MODE_INPUT(base, pin)                                           \
  CY_SYS_PINS_SET_DRIVE_MODE(PORT_ADDRESS(pin) + 8, PIN_IN_PORT(pin),          \
...
static inline __attribute__((always_inline)) void
directWriteLow(IO_REG_TYPE mask) {
  GPIO_REG(GPIO_OUTPUT_VAL) &= ~mask;
}

static inline __attribute__((always_inline)) void
directWriteHigh(IO_REG_TYPE mask) {
  GPIO_REG(GPIO_OUTPUT_VAL) |= mask;
}
...
#define interrupts()                                                           \
  osThreadSetPriority(osThreadGetId(),                                         \
                      osPriorityNormal) // core_util_critical_section_exit()
